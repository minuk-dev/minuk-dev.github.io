---
lastmod: 2026-07-05 12:47:46 +0900
date: 2026-06-27 16:36:33 +0900
---
- 한글판: 좋아보여
# Part 1. 코드 리뷰의 기초 지식
...
#### 3.3.3. PR 승인을 막는 요인
- 허용 요인 이슈: 별도의 PR이나 건설적인 피드백으로 해결이 가능하여 PR승인을 차단해서는 안 되는 이슈
	- 스타일 선호
	- 사소한 서식의 불일치
	- 문서의 사소한 문제
	- 누락된 선택적 기능
	- 경미한 리팩터링 기회
	- 관련 없는 개선사항
- 차단 요인 이슈: 해결이 필요하여 PR 승인을 차단해야 하는 이슈
	- 핵심 기능
	- 보안 문제
	- 코드 표준 위반
	- 깔끔하지 않은 코드
...
#### 3.3.4. 승인 정책
- 누가 PR을 승인할 수 있는가?
	- 이성적으로는 개발팀의 모든 구성원이 PR을 승인할수 있어야한다.
	- 매니저, 기술 리드, 프로그램 매니저, QA 엔지니어 등?
	- 정보 제공 목적인 리뷰어에게도 완전한 승인 권한을 부여할까?
	- 외부팀도 PR을 승인할 수 있을까?
	- 리뷰어 수를 인위적으로 늘려 너무 많은 사람이 리뷰를 하게 되면 다른 사람에게 떠넘길 가능성이 커질수 있다.
	- 비상 상황에서 승인 권한을 가진 인원을 제한할까?
- 얼마나 많은 승인이 필요할까?
	- 최소 2명의 리뷰어 승인을 강력히 권장한다.

3단계 결과물
- 네 가지 필수 가이드라인 설정
	- 워크플로: 코드 리뷰 세부 단계를 계획하고 개요를 작성한다.
	- 리뷰의 중점: 코드 리뷰 중 집중할 사항을 정의한다.
	- 허용 vs. 차단 이슈: PR 승인을 허용할 수 있는 문제와 차단할 수 있는 문제를 명확히 구분한다.
	- 승인 정책: 누가 PR을 승인할 수 있는지와 필요한 승인자의 수를 결정한다.
...
# Part 2. 고급 코드 리뷰를 위한 필수 요소
## Chapter 2. 팀 워킹 어그리먼트
### 4.1. 팀 워킹 어그리먼트란?
- TWA: Team Working Agreement
- 암묵적인 사회적 기대를 명확하고 강제하는 규칙으로 전환한다.
- 자동화를 할 수 있는 부분은 반드시 자동화를 해야 한다

### 4.2. 팀 워킹 어그리먼트로 팀 기대치 설정하기
- 합리적인 PR 응답 시간
- 명명 규칙과 표준
- 차단 이슈와 허용 이슈
- 팀 스타일 가이드
...
### 4.3. 팀 워킹 어그리먼트 수립하기
- 처음에는 역할의 행동 기대치에 대한 의견을 요청하는 것이 어색하게 느껴질 수도 있다.
- 공감하는 태도와 체계적인 방식으로 논의한다면 팀의 가장 가치 있는 인사이트를 얻을 수 있는 기회가 될 수 있다.

#### 4.3.1. 팀에 팀 워킹 어그리먼트가 꼭 필요할까?
- 구글의 엔지니어링 방법론:
	- https://soojin.ro/blog/google-code-review-guide
- gitlab 코드리뷰 가이드라인: https://docs.gitlab.com/development/code_review/
- 미국 GSA(연방총무청) 기술 혁신 서비스 가이드: https://handbook.18f.org/18f/chapters/engineering/
- Yelp Engineering 코드 리뷰 가이드라인: https://engineeringblog.yelp.com/2017/11/code-review-guidelines.html